...
curr_train_ratio = 0.005
Scale = 100
data_mat = sio.loadmat('../input/hybridsn/data/Salinas_corrected.mat')
data = data_mat['salinas_corrected']
gt_mat = sio.loadmat('../input/hybridsn/data/Salinas_gt.mat')
...
class ViT(nn.Module):
    def __init__(self, *,in_channels=1,image_size, patch_size, num_classes, dim, depth, heads,
        ...
        self.conv2d_f = nn.Sequential(
            nn.Conv2d(in_channels=32*174,out_channels=64,kernel_size=(3,3),padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        )
        ...
        self.conv2d_features = nn.Sequential(
            nn.Conv2d(in_channels=64 * 174, out_channels=64, kernel_size=(3, 3), padding=1),
            nn.BatchNorm2d(64),
            nn.ReLU(),
        )
        self.conv2d_features_1 = nn.Sequential(
            nn.Conv2d(in_channels=11200, out_channels=110, kernel_size=(3, 3), padding=1),
            nn.BatchNorm2d(110),
            nn.ReLU(),
        )
...
def train(train_loader, epochs):
    ...
    net = ViT(image_size = 13,patch_size = 1,num_classes = NUM_CLASS,dim = 1024,depth = 2,
              heads = 16,mlp_dim = 2048,channels =174,dropout = 0.1,emb_dropout = 0.1).to(device)
    # 交叉熵损失函数
...
if __name__ == '__main__':
    ...
    # 只保存模型参数
    torch.save(net.state_dict(),'SA_net_params.pth')
    ...
    file_name = "./HSI_classification_SA_report_"+str(localtime)+".txt"
